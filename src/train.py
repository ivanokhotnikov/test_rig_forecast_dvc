...
def train(local_train_data_path, local_models_path, local_metrics_path,
          feature):
    """Defines and trains the model, saves the model and its metrics to the specified paths.

    Args:
        local_train_data_path (str): Train data directory
        local_models_path (str): Models directory
        local_metrics_path (str): Metrics directory
        feature (str): Feature to train for
    """
    params = params_show()
    ...
    dump(scaler, os.path.join(local_models_path, f'{feature}.joblib'))
    ...
    with open(os.path.join(local_metrics_path, f'train_{feature}.json'),
              'w') as f:
        for k, v in history.history.items():
            history.history[k] = [float(vi) for vi in v]
        f.write(json.dumps(history.history))
    forecaster.save(os.path.join(local_models_path, f'{feature}.h5'))
